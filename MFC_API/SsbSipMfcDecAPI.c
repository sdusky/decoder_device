    ...
    while (1) {
        while (startCode != USR_DATA_START_CODE) {
            if (startCode == VOP_START_CODE) {
                printf("isPBPacked: VOP START Found !!.....return\n");
                printf("isPBPacked: Non Packed PB\n");
        ...
        printf("isPBPacked: User Data Found !!\n");

        do {
            if (*strmBuffer == 'p') {
                printf("isPBPacked: Packed PB\n");
...
void *SsbSipMfcDecOpen(void *value)
{
    ...
    if (pCTX == NULL) {
        fprintf(stderr,"SsbSipMfcDecOpen: malloc failed.\n");
        return NULL;
    }
    ...
    if (hMFCOpen < 0) {
        fprintf(stderr,"SsbSipMfcDecOpen: MFC Open failure\n");
        return NULL;
    }
    ...
    if (!mapped_addr) {
        fprintf(stderr,"SsbSipMfcDecOpen: FIMV5.0 driver address mapping failed\n");
        return NULL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecInit(void *openHandle, SSBSIP_MFC_CODEC_TYPE codec_type, int Frameleng)
{
    ...
    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecSetConfig: openHandle is NULL\n");
    ...
    if ((codec_type != MPEG4_DEC)  &&
        ...
        (codec_type != VC1_DEC)) {
        fprintf(stderr,"SsbSipMfcDecOpen: Undefined codec type.\n");
        return MFC_RET_INVALID_PARAM;
    }
    ...
    if (DecArg.ret_code != MFC_RET_OK) {
        fprintf(stderr,"SsbSipMfcDecInit: IOCTL_MFC_DEC_INIT (%d) failed\n", DecArg.ret_code);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecExe(void *openHandle, int lengthBufFill)
{
    ...
    if (openHandle == NULL) {
        //fprintf(stderr,"SsbSipMfcDecExe: openHandle is NULL\n");
        return MFC_RET_INVALID_PARAM;
    }

    if ((lengthBufFill < 0) || (lengthBufFill > MAX_DECODER_INPUT_BUFFER_SIZE)) {
        //fprintf(stderr,"SsbSipMfcDecExe: lengthBufFill is invalid. (lengthBufFill=%d)\n", lengthBufFill);
        return MFC_RET_INVALID_PARAM;
    }
    ...
    if (DecArg.ret_code != MFC_RET_OK) {
        //fprintf(stderr,"SsbSipMfcDecExe: IOCTL_MFC_DEC_EXE failed(ret : %d)\n", DecArg.ret_code);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecClose(void *openHandle)
{
    ...
    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecClose: openHandle is NULL\n");
        return MFC_RET_INVALID_PARAM;
    }
...
void *SsbSipMfcDecGetInBuf(void *openHandle, void **phyInBuf, int inputBufferSize)
{
    ...
    if (inputBufferSize < 0) {
        fprintf(stderr,"SsbSipMfcDecGetInBuf: inputBufferSize = %d is invalid\n", inputBufferSize);
        return NULL;
    }

    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecGetInBuf: openHandle is NULL\n");
        return NULL;
    }
    ...
    if (ret_code < 0) {
        fprintf(stderr,"SsbSipMfcDecGetInBuf: IOCTL_MFC_GET_IN_BUF failed\n");
        return NULL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetInBuf(void *openHandle, void *phyInBuf, void *virInBuf, int inputBufferSize)
{
    ...
    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecSetInBuf: openHandle is NULL\n");
        return MFC_RET_INVALID_PARAM;
    }
...
SSBSIP_MFC_DEC_OUTBUF_STATUS SsbSipMfcDecGetOutBuf(void *openHandle, SSBSIP_MFC_DEC_OUTPUT_INFO *output_info)
{
    ...
    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecGetOutBuf: openHandle is NULL\n");
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    if (value == NULL) {
        fprintf(stderr,"SsbSipMfcDecSetConfig: value is NULL\n");
        return MFC_RET_INVALID_PARAM;
    }
    ...
    default:
        fprintf(stderr,"SsbSipMfcDecSetConfig: No such conf_type is supported.\n");
        return MFC_RET_INVALID_PARAM;
    }

    ret_code = ioctl(pCTX->hMFC, IOCTL_MFC_SET_CONFIG, &DecArg);
    if (DecArg.ret_code != MFC_RET_OK) {
        fprintf(stderr,"SsbSipMfcDecSetConfig: IOCTL_MFC_SET_CONFIG failed(ret : %d, conf_type: %d)\n", DecArg.ret_code, conf_type);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecGetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    if (openHandle == NULL) {
        fprintf(stderr,"SsbSipMfcDecGetConfig: openHandle is NULL\n");
        return MFC_RET_FAIL;
    }

    if (value == NULL) {
        fprintf(stderr,"SsbSipMfcDecGetConfig: value is NULL\n");
        return MFC_RET_FAIL;
    }
    ...
    case MFC_DEC_GETCONF_CRC_DATA:
        ...
        if (DecArg.ret_code != MFC_RET_OK) {
            fprintf(stderr,"SsbSipMfcDecGetConfig: IOCTL_MFC_GET_CONFIG failed(ret : %d, conf_type: %d)\n", DecArg.ret_code, conf_type);
    ...
    default:
        fprintf(stderr,"SsbSipMfcDecGetConfig: No such conf_type is supported.\n");
        return MFC_RET_INVALID_PARAM;
    }
